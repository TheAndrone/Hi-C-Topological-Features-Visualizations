...
class myContainer {
  constructor(playground) {
    ...
    this.d = {}; //dimensions
    //this.addButton(); //the debug button
    this.ndx;
  }
  ...
  afterShowChrSelect() {
    //called before main function, prepares all fields
    addCheck(this.allData["tissues"]); //add checkboxes for tissues
    uncheckAllTicks();
    this.createNumberDivs(); //create DIVs for tissue clique statistics

    //get all possible tisBitMask values for current chromosome - only masks in input data file are added
  ...
  startCrossfiltering() {
    ...
    myClusterChart //CLUSTER ACTUALLY
      ...
      // .colors(function(colorKey) { return plotColorMap[colorKey+1]; }); //old way for choosing colors
    ...
    ext[1] += 5; //to be careful around the edges
    ext[0] = 0;
    EXTT[1] = ext[1];
    myCliqueLociPlotC
      .width(1000)
  ...
  showTissues() {
    //Sandra's version for creating tissue checkboxes. nu
    var self = this;
  ...
  selectedTissues() {
    //Sandra's version of creating checkboxes. nu
...
function remove_bins(source_group, bins) {
  ...
  //alternative to make sure ndx remains intact
  return {
    all: function () {
